...
namespace LightGBM
{
  ...
  ObjectiveFunction *ObjectiveFunction::CreateObjectiveFunction(const std::string &type, const Config &config)
  {
        ...
        config.boosting != std::string("rf"))
    {
      ...
      else if (type == std::string("probit"))
      {
        Log::Warning("Objective probit is not implemented in cuda version. Fall back to boosting on CPU.");
        return new ProbitLogloss(config);
      }
