...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  ...
  public:
        ...
        : deterministic_(config.deterministic)
    {
      ...
      if (link_name == "canonical")
      {
        if (distribution_name == "guassian")
        {
          link_ = std::shared_ptr<IdentityLink>(new IdentityLink());
        }
        else
        {
          Log::Fatal("Unknown canonical link for distribution %s", distribution_name.c_str());
        }
      }
      ...
      {
        link_ = std::shared_ptr<LogLink>(new LogLink());
      }
      else if (link_name == "log1p")
      {
        link_ = std::shared_ptr<Log1pLink>(new Log1pLink());
      }
      else if (link_name == "sqrt")
      {
        link_ = std::shared_ptr<SqrtLink>(new SqrtLink());
      }
      else if (link_name == "inverse")
      {
        link_ = std::shared_ptr<InverseSquareLink>(new InverseSquareLink());
      }
      else if (link_name == "power")
      {
        link_ = std::shared_ptr<PowerLink>(new PowerLink());
      }
      else if (link_name == "powerabs")
      {
        link_ = std::shared_ptr<PowerAbsLink>(new PowerAbsLink());
      }
      else
      {
        Log::Fatal("Unknown expenential regression link function %s", link_name.c_str());
      }

      if (distribution_name == "guassian")
      {
        distribution_ = std::unique_ptr<Guassian>(new Guassian(link_));
      }
      else
      {
        ...
        : deterministic_(false)
    {
      ...
      // TODO: parse power parameter here
      for (auto str : strs)
      {
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
        for (data_size_t i = 0; i < num_data_; ++i)
        {
          gradients[i] = static_cast<score_t>(distribution_->NegativeLogLikelihoodFirstDerivative(label_[i], score[i]));
          hessians[i] = static_cast<score_t>(distribution_->NegativeLogLikelihoodSecondDerivative(label_[i], score[i]));
        }
      }
      else
      {
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
        for (data_size_t i = 0; i < num_data_; ++i)
        {
          gradients[i] = static_cast<score_t>(distribution_->NegativeLogLikelihoodFirstDerivative(label_[i], score[i])) * weights_[i];
          hessians[i] = static_cast<score_t>(distribution_->NegativeLogLikelihoodSecondDerivative(label_[i], score[i])) * weights_[i];
        }
      }
    }
    ...
    bool IsConstantHessian() const override
    {
      return weights_ == nullptr && strcmp(link_->GetName(), "identity") == 0;
    }
